...
export function exportProductsToCSV(products: ExtendedProduct[], filename = "products.csv"): void {
  const data = products.map((product) => ({
    ...
    Amazon商品名: product.asin?.amazon_name,
    Amazon価格: product.asin?.amazon_price,
    月間販売数: product.asin?.monthly_sales,
    手数料率: product.asin?.fee_rate,
    FBA手数料: product.asin?.fba_fee,
