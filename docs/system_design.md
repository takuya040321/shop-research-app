...
- **ProductTable**: 商品テーブルコンポーネント（Shift+ホイールで横スクロール対応、お気に入り機能付き、行レベル更新対応、Toast通知統合）
  - **useProductTable**: 統合カスタムフック（データ管理、UI制御、Toast通知を一元化）
    ...
    - 横スクロール機能（Shift+ホイール）
    - コンテキストメニュー項目生成
    - **編集後フィルター除外機能**: 編集した商品はフィルター範囲外でも表示継続し、「適用」ボタンまたはページリロードで再適用
  - **useProductSearch**: 商品検索・フィルター管理カスタムフック
    - **一時フィルターパターン**: フィルター変更は即座に反映せず、「適用」ボタンで確定
    - 検索テキスト（即時反映）
    - 価格範囲フィルター（最低価格、最高価格）
    - 利益率範囲フィルター
    - ROI範囲フィルター
    - 月間売上ステータスフィルター（すべて/データあり/データなし）
    - 月間売上範囲フィルター
    - ASINステータスフィルター（すべて/設定済み/未設定）
    - お気に入りフィルター（すべて/お気に入りのみ/お気に入り以外）
    - セール状況フィルター（すべて/セール中のみ/通常価格のみ）
    - 表示状態フィルター（すべて/表示中のみ/非表示のみ）
    - Amazon販売フィルター（すべて/ありのみ/なしのみ）
    - 公式販売フィルター（すべて/ありのみ/なしのみ）
    - クレーム数フィルター（すべて/クレームあり/クレームなし）
    - 危険品フィルター（すべて/該当のみ/非該当のみ）
    - パーキャリNGフィルター（すべて/該当のみ/非該当のみ）
    - アクティブフィルター数のカウント表示
  - **ProductSearch**: 商品検索・フィルターUIコンポーネント
    - 検索バー（商品名、ASIN、Amazon商品名）
    - フィルターボタン（アクティブフィルター数バッジ表示）
    - クリアボタン（アクティブフィルターがある場合のみ表示）
    - 折りたたみ式フィルターパネル（14種類のフィルター）
    - キャンセル/適用ボタン（適用時にフィルター確定 + 編集済み商品追跡をクリア）
